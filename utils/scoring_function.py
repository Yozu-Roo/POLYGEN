...
class SegmentLinearThresholdModifier(ScoreModifier):
    """
    Score modifier that return 1 if greater than the threshold,
    otherwise return the linearly mapped value.
    """

    def __init__(self, threshold=1.0, slope=1.0):
        self.slope = slope
        self.threshold = threshold

    def __call__(self, x):
        x = self.slope * x if x < self.threshold else 1.0
        # print("modify:",x)
        return x
...
class MoleculewiseScoringFunction(ScoringFunction):
    ...
    def score_list(self, smiles_list: List[str]) -> List[float]:
        return [self.score(smiles) for smiles in tqdm(smiles_list, desc=self.__class__.__name__, position=1, leave=False)]
...
class ArithmeticMeanScoringFunction(BatchScoringFunction):
    ...
    def __init__(self, scoring_functions: List[ScoringFunction], weights=None, threshold=None) -> None:
        """
        Args:
        ...
        self.threshold = np.zeros(number_scoring_functions) if threshold is None else threshold
    ...
    def raw_score_list(self, smiles_list: List[str]) -> List[float]:
        ...
        for function, weight in tqdm(zip(self.scoring_functions, self.weights), desc="Scoring", position=0):
            res = function.score_list(smiles_list)
            scores.append(weight * np.array(res))
            raw_scores.append(np.array(res))
