...
class ToxicityScore(MoleculewiseScoringFunction):
    ...
    def __init__(self, model, scaler):
        ...
        self.model = model
        self.scaler = scaler
        self.names = ['MolWt', 'TPSA', 'NumHDonors', 'NumHAcceptors', 'MolLogP', 'HeavyAtomCount', 'NumRotatableBonds',
                      'RingCount']
...
class LatentDistance(MoleculewiseScoringFunction):
    ...
    def __init__(self, finetune_dataset, trainer, score_modifier, n_top=None, agg="mean"):
        super().__init__(score_modifier=score_modifier)
        self.trainer = trainer
        self.config = finetune_dataset.config
        self.z_targets = self.trainer.calculate_latent_from_dataset(self.config)


        if n_top is None:
            self.n_top = len(finetune_dataset)
        else:
            self.n_top = int(n_top)
        self.agg = agg
    ...
    def raw_score(self, smiles):
        """ Ge distance to set """
        z = self.trainer.calculate_latent_from_smiles(smiles, self.config)
...
class BindingScorer(MoleculewiseScoringFunction):
    def __init__(self, receptor_name, score_modifier=None):
        super().__init__(score_modifier=score_modifier)
        if receptor_name == "GSK3B":
            self.calculator = GSK3BCalculator()
        elif receptor_name == "JNK3":
            self.calculator = JNK3Calculator()

    def raw_score(self, smiles: str) -> float:
        mol = Chem.MolFromSmiles(smiles)
        return self.calculator.get_score(mol)

class BindingScorer_Custom(MoleculewiseScoringFunction):
    def __init__(self, score_modifier, model_path):
        super().__init__(score_modifier=score_modifier)
        with open(model_path, 'rb') as handle:
            self.rfr = pickle.load(handle)
