...
def train_fn(train_loader, model, optimizer, epoch, scheduler, beta=1, scaler=None):
    ...
    for step, batch_data in tqdm(enumerate(train_loader), disable=disable_tqdm, total=N):
        ...
        mask = mappings != -100
        mapping_loss = F.binary_cross_entropy(mapping_scores[mask], mappings[mask], weight=mapping_loss_weight[mask])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='./pretrain_output')
